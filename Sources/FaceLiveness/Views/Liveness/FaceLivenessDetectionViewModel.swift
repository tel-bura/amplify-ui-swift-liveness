...
class FaceLivenessDetectionViewModel: ObservableObject {
    ...
    func drawOval(onComplete: @escaping () -> Void) {
        ...
        let ovalWidthMid = ovalParameters.boundingBox.width / 2
        let ovalHeightMid = ovalParameters.boundingBox.height / 2

        print("ovalWidthMid \(ovalWidthMid)")
        ...
        let rect = CGRect(
            x: screenHeightMid - (ovalHeightMid / 2), // 213.25
            y: screenWidthMid - (ovalWidthMid / 2), // 406.5
            width: ovalWidthMid,
        ...
        livenessViewControllerDelegate?.drawOvalInCanvas(rect)
        DispatchQueue.main.async {
